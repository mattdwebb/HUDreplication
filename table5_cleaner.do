/* City Name Cleaner File for Table 5 */
...
/* does the city match the acceptable city name */
	replace good_city = 1 if hcity == primary_city
	replace good_city = 1 if hcity == officialuspscityname

/*loop over all the acceptable cities and counties*/
	forvalues i = 1 /16{
		replace good_city = 1 if hcity == 	allaccept`i'
	}

	forvalues i = 1 /6{	
		replace good_city = 1 if hcity == 	allcounty`i'
	}

/*now to clean the strings*/
...
/* tries to match on splits of the typed city name and the primary city name */
	...
	replace good_city = 1 if  clean_hcity == "MORRISTWP"
