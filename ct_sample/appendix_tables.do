...
foreach APPENDIX_TABLE_MODE in `APPENDIX_TABLE_MODES' {
    if "`APPENDIX_TABLE_MODE'" == "corrected" {
        ...
        local APPENDIX_MODE_OUTPUT "`APPENDIX_BASE_OUTPUT'/corrected"
    }
        ...
        local APPENDIX_MODE_OUTPUT "`APPENDIX_BASE_OUTPUT'/original"
    }
    else {
    ...
    display as text "Generating C&T-sample appendix tables for mode: " as result "`APPENDIX_TABLE_MODE'" as text " into ${OUTPUT}"
...
if "`APPENDIX_ANALYSIS_TYPE'" == "corrected" {
    display as text "Skipping Table 13 for corrected drop-trial-invariant-controls specification."
}
else {
