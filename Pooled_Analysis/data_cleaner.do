...
// Section 1: Clean up zip codes and merge data
	...
	// Merge city data
	merge m:1 zip using "${DATA}/CT2022_Replication_Data/zipinfo.dta" // Merge with zip data
	drop if _merge==2 // Drop unmatched entries from zip file
	drop _merge
	drop allaccept17-allaccept31 // Drop unneeded alternatives

	// Merge county data
	merge m:1 zip using "${DATA}/CT2022_Replication_Data/zipinfo-county.dta" // Merge with county data matched to zip code
	drop if _merge==2 // Drop unmatched entries from zip file
	drop _merge
