...
setwd("/Users/shichen/Desktop/HUDrep_block_update")
...
# this is the output of 'data_censor.R', not the original file
...
test <- adsprocessed_data_processed %>% 
   # select(HCITY, GEOID10, CONTROL, STATEFP10, COUNTYFP10, block_geoid, place_geoid, place_name, coverage_pct) %>%
   select(HCITY, GEOID10, CONTROL, STATEFP10, COUNTYFP10, block_geoid, place_geoid, place_name) %>%
...
selected_tracts <- block_place_mappings %>%
  filter(block_geoid %in% c("39055312203", "39055312202"))
